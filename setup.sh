...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing Illusionsms'
    else
        figlet Illusionsms
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[R447H \e[0m"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border IlluzX
    fi
...
install_deps(){
    ...
    if [ -n "$INSTALL" ];then
        ...
        echo "Then you can execute sms.py ."
        exit
    fi
}
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN Illusionsms Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
