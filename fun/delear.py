...
class APIPov:
    ...
    def __init__(self, cc, target, mode, delay=0):
        try:
            ...
            PROVIDERS = requests.get(
                "https://github.com/illuzX/Illusionsms/raw/main/apidata.json"
            ).json()
        self.config = None
        self.cc = cc
        self.target = target
        ...
        APIPov.api_providers = providers.get(cc, [])
        if len(APIPov.api_providers) < 10:
            APIPov.api_providers += providers.get("multi", [])
    ...
    def select_api(self):
        try:
            if len(APIPov.api_providers) == 0:
                raise IndexError
            self.index += 1
            if self.index >= len(APIPov.api_providers):
                self.index = 0
        except IndexError:
            self.index = -1
            return
        self.config = APIPov.api_providers[self.index]
    ...
    def remove(self):
        try:
            del APIPov.api_providers[self.index]
            return True
        except Exception:
            return False
    ...
    def hit(self):
        try:
            if not APIPov.status:
                return
            time.sleep(APIPov.delay)
            self.lock.acquire()
